...
public class BinarySearchTreeTeacher {
    ...
    public static void main(String[] args) {
        BinarySearchTreeTeacher BinarySearchTreeTeacher = new BinarySearchTreeTeacher();
        ...
        while (true) { //
            ...
            if (data == -1)
                break;
            BinarySearchTreeTeacher.insert(root, data);
            t++;
            System.out.println("请输入第" + t + "个点:输入-1表示结束");
        }
        BinarySearchTreeTeacher.show(root);		//找的别人写的打印二叉树形结构，感觉还不错，可以更加清晰
        System.out.println("删除测试:");
        while(true) {
            System.out.println("请输入要删除的点：-1表示结束");
            int key = cin.nextInt();
            root = BinarySearchTreeTeacher.remove(root, key);
            BinarySearchTreeTeacher.show(root);
            if(root == null) {
